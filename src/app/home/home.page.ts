...
import { ToastController, ModalController, IonItemSliding, Platform,IonList } from '@ionic/angular';
...
export class HomePage implements OnInit {
  ...
  async imprimir(){
    ...
    this.tablaAporteService.reporte(this.detalles).subscribe(response=>{
      ...
      this.file.writeFile(writeDirectory, response.filename, this.convertBaseb64ToBlob(response.file, 'application/pdf'), {replace: true})
        .then(() => {
            this.loadingService.dismiss();
            toast.present();
